...
?>
	<title>SCOTUS 2015 Term Networks</title>
...
<div class="container">
  <div class="page-header">
    <h1 class="text-center"><span class="text-danger">2015 Term</span> SCOTUS Networks</h1>      
  </div>
</div>
...
<div id="main">
	...
	<p>
		This collection of 2015 Term Supreme Court citation networks was created with a <a class="external" href="https://courtlistener.com/visualizations/scotus-mapper/">free online tool</a> available on <a class="external" href="https://courtlistener.com/visualizations/scotus-mapper/">CourtListener</a>. Every case decided by the Court in its 2015 Term is in the table below with links to CourtListener network visualizations and <a href="http://www.scotusblog.com/case-files/terms/ot2015/">SCOTUSBlog</a> entries. You can sort table data by column values including network age, number of network cases, and degree of dissent. Read more about our terminology and method below. This collection is part of the <a class="external" target="_blank" href="http://law.ubalt.libguides.com/scotuslib">SCOTUS Map Library</a> developed by the <a class="external" href="http://law.ubalt.edu/faculty/scotus-mapping/index.cfm">Supreme Court Mapping Project</a>. 
	</p>
		
	<p class="text-center">
		<a href="#NetworkTerms">Network Terminology</a> ~ <a href="#Method">Method</a> ~  <a href="#TermStats">2015 Term Statistics</a> ~ <a href="#Collaborate">Collaborate</a>
	</p>
  	
	<a name="2015Data"></a>
	<h3>2015 Term Network Data</h3>

	<table id="tablesorter-scotus" class="tablesorter" border="0" cellpadding="0" cellspacing="1">
		<thead>
			<tr>
				<th>2015 Term Case (link to CourtListener Visualization)</th>
				<th>Decided</th>
...
<!-- Network Terms, 2015 Term Stats, Method/Analysis, and Collaborate -->
	<a name="TermStats"></a>
	<h3>2015 Term Statistics</h3>
	<p>
	<ul>
		<li>Number of networks from 2015 Term: <strong><?php echo $term_2015->number_networks(); ?></strong>. Note: 2015 Term cases where no precedent was cited were not included in this collection.</a>. </li>
		
		<li>Average age of network:<strong><?php echo $term_2015->network_average_age_years();?> years</strong>. Median age of network:<strong><?php echo $term_2015->network_median_age_years();?> years</strong>.</li>
		<li>Average number of 3D cases:<strong><?php echo $term_2015->network_average_num_3D_cases();?></strong>. Median number of 3D cases:<strong><?php echo $term_2015->network_median_num_3D_cases();?></strong>.</li>
		<li>Average number of 2D cases:<strong><?php echo $term_2015->network_average_num_2D_cases();?></strong>. Median number of 2D cases:<strong><?php echo $term_2015->network_median_num_2D_cases();?></strong>.</li>
		<li>Average 3D Degree of Dissent:<strong><?php echo $term_2015->network_average_3D_DOD();?></strong>. Median 3D Degree of Dissent:<strong><?php echo $term_2015->network_median_3D_DOD();?></strong>.</li>
		<li>Average 2D Degree of Dissent:<strong><?php echo $term_2015->network_average_2D_DOD();?></strong>. Median 2D Degree of Dissent:<strong><?php echo $term_2015->network_median_2D_DOD();?></strong>.</li>
	</ul>
	</p>
	...
	<p>
		<ul>
			<li>Supreme Court citation networks show the citation connections between two Court cases. Every network connects two cases, the <strong>2015 Term Case</strong> and the <strong>Network Anchor Case</strong>.</li>
			<li>The <strong>Network Age</strong> expresses the amount of time (in years) between the time the Anchor Case was decided and when the 2015 Term Case was decided.</li>
		  	<li>The <strong>"D"</strong> in <strong>"3D"</strong> and <strong>"2D"</strong> stands for "degree of connection." Since the 2015 Term Case always cites the Network Anchor Case, those cases are 1-degree connected.</li> 
		  	<li>The <strong>2D cases</strong> column gives the number of 2-degree connections that exist between the 2015 Term Case and the Network Anchor Case - that is, how many cases exist in the network that are cited by the 2015 Term Case AND in turn cite the Network Anchor. For example, Case X would be a 2D case if the 2015 Term Case cited Case X and Case X in turn cited the Network Anchor Case. Please note that the 2D case number includes the two 1D cases (2015 Term Case + Network Anchor).</li>
		  	<li>The <strong>3D cases</strong> column gives the number of 3-degree connections that exist between the 2015 Term Case and the Network Anchor Case. 3D cases are those that are cited by a 2D case AND in turn cite the Network Anchor. Thus, Case Y would be a 3D case if Case X above Cited Y, which in turn cited the Network Anchor Case. Please note that the 3D case number includes the all the 2D+1D cases.</li>
	...
	<h3>Contact</h3>
	<p>
		This collection of 2015 Term cases was created as part of the <a class="external" href="http://law.ubalt.edu/faculty/scotus-mapping/index.cfm">Supreme Court Mapping Project</a>. We welcome feedback and will link <a class="external" href="https://courtlistener.com/visualizations/scotus-mapper/">networks you create on CourtListener</a> to visualizations presented in this table. Reach us by <a href="mailto:cstarger@ubalt.edu?subject=2015%20Term%20Contact">email.</a>
	</p>
	<p class="text-center">
